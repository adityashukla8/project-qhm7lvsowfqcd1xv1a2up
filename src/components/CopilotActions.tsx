  ...
  useCopilotAction({
    ...
    handler: async ({ patientId }) => {
      try {
        ...
        // Call the fetchPatients function with the patient ID
        const response = await fetchPatients({ patientId: patientId.trim() });
        console.log('CopilotKit: Patient search response:', response);
...
- Histology: ${patient.histology || 'Not specified'}
- Biomarker: ${patient.biomarker || 'Not specified'}
...
- Number of matched trials: ${patient.matched_trials_count || 0}`;
          ...
          console.log('CopilotKit: Patient not found or API error:', response);
  ...
  useCopilotAction({
    ...
    handler: async () => {
      try {
        ...
        // Call fetchPatients without patientId to get all patients
        const response = await fetchPatients({});
        console.log('CopilotKit: Patient stats response:', response);
...
🏥 **Top Conditions:**
${topConditions || 'No conditions data available'}
...
🎯 **Match Rate:** ${processedPatients > 0 ? Math.round((matchedPatients / processedPatients) * 100) : 0}%`;
          ...
          console.log('CopilotKit: No patient data found:', response);
          return `❌ Unable to retrieve patient statistics. ${response?.error || 'No data available'}`;
        }
      } catch (error) {
  ...
  useCopilotAction({
    ...
    handler: async ({ trialId }) => {
      try {
        ...
        // Call the trialInfo function with the trial ID
        const response = await trialInfo({ trial_id: trialId.trim() });
        console.log('CopilotKit: Trial search response:', response);
        
        if (response && response.success && response.trial_info) {
...
- Matched Patients: ${trial.matched_patients_count || 0}`;
          ...
          console.log('CopilotKit: Trial not found or API error:', response);
          return `❌ Trial with ID "${trialId}" not found. Please check the trial ID and try again. ${response?.error || ''}`;
        }
      } catch (error) {
  ...
  useCopilotAction({
    ...
    handler: async () => {
      try {
        ...
        // Call fetchTrials to get all trials
        const response = await fetchTrials();
        console.log('CopilotKit: Trial stats response:', response);
        
        if (response && response.success && response.data?.trials) {
...
🔬 **Phase Distribution:**
${phaseBreakdown || 'No phase data available'}

📈 **Average Matches per Trial:** ${totalTrials > 0 ? Math.round(totalMatches / totalTrials * 10) / 10 : 0}`;
        } else {
          console.log('CopilotKit: No trial data found:', response);
          return `❌ Unable to retrieve trial statistics. ${response?.error || 'No data available'}`;
        }
      } catch (error) {
